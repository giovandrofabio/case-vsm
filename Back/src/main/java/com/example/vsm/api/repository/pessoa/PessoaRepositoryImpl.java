...
public class PessoaRepositoryImpl implements PessoaRepositoryQuery{
	...
	private Predicate[] criarRestricoes(PessoaFilter pessoaFilter, CriteriaBuilder criteriaBuilder,
		...
		if (StringUtils.hasLength(pessoaFilter.getNome())) {
			predicates.add(criteriaBuilder.like(criteriaBuilder.lower(root.get("nome")), "%" + pessoaFilter.getNome().toLowerCase() + "%"));
		}
		
		if (pessoaFilter.getCpfcnpj() != null) {
			predicates.add(criteriaBuilder.equal(root.get("cpfcnpj"), pessoaFilter.getCpfcnpj()));
		}
		
		if (pessoaFilter.getTipo() != null) {
			predicates.add(criteriaBuilder.equal(root.get("tipo"), pessoaFilter.getTipo()));
		}
